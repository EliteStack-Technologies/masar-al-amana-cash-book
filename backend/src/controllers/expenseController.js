...
export const getExpense = asyncHandler(async (req, res) => {
  const expense = await Expense.findOne({ _id: req.params.id, shopOwner: req.user._id }).lean();
...
export const updateExpense = asyncHandler(async (req, res) => {
  const expense = await Expense.findOne({ _id: req.params.id, shopOwner: req.user._id });
  if (!expense) return res.status(404).json({ message: 'Expense entry not found' });
...
export const deleteExpense = asyncHandler(async (req, res) => {
  const expense = await Expense.findOneAndDelete({ _id: req.params.id, shopOwner: req.user._id });
  if (!expense) return res.status(404).json({ message: 'Expense entry not found' });
