...
import { IconArrowUp, IconSearch, IconChevron, IconPlus, IconDownload } from '@/components/Icons';
...
export default function IncomePage() {
  ...
  return (
    <AppShell
      ...
      action={
        <Link href="/income/new" aria-label="Add income" className="flex size-9 items-center justify-center border border-[var(--rule-strong)] active:bg-[var(--paper-2)]">
          <IconPlus size={18} />
        </Link>
      }
    >
      <div className="space-y-4">
        ...
        {!data && !error ? (
          ...
          <div className="space-y-4 rise">
            <Card className="p-3.5"><Figure label="Total income (filtered)" value={money(data.totals.amount)} tone="leaf" size="lg" /></Card>

            <div className="flex gap-2.5">
              {['excel', 'pdf'].map((f) => (
                <a key={f} href={downloadUrl(f, { type: 'income' })} target="_blank" rel="noreferrer" className="flex-1">
            ...
            <div className="card ruled py-0">
              {data.items.map((it) => (
                <Link key={it._id} href={`/income/${it._id}/edit`} className="flex items-center gap-3 px-3.5 py-3 active:bg-[var(--paper-2)]">
                  ...
                  <p className="sum shrink-0 text-[15px] text-leaf-500 dark:text-leaf-400">+{money(it.amount)}</p>
