          ...
          <Card className="p-0">
            <div className="grid grid-cols-2">
              <div className="border-r border-[var(--rule)] p-3.5">
                <Figure label="Total put in" amount={t.invested} size="lg" />
              </div>
              <div className="p-3.5">
                <Figure label="In the shop" amount={t.balance} tone="leaf" size="lg" />
              </div>
            </div>
                  ...
                  <Card key={c._id} className="p-0">
                    <Link href={`/capital/${c._id}`} className="flex items-center gap-3 p-3.5 active:bg-[var(--paper-2)]">
                      ...
                      <div className="shrink-0 text-right">
                        <p className="sum text-[13px] text-stamp-500 dark:text-stamp-400"><Amt value={c.withdrawnAmount} /> out</p>
                        <p className="sum text-[13px] text-leaf-500 dark:text-leaf-400"><Amt value={c.balance} /> in</p>
                      </div>
                      ...
                      <div className="border-t border-[var(--rule)] px-3.5 py-2">
                        ...
                        {c.withdrawals.map((w) => (
                          <div key={w._id} className="flex items-baseline justify-between gap-3 py-0.5">
                            ...
                            <p className="sum shrink-0 text-[12px] text-stamp-500 dark:text-stamp-400"><Amt value={w.amount} /></p>
                          </div>
                        ))}
                      </div>
                    )}
