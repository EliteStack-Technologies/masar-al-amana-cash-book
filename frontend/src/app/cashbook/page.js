...
export default function CashbookPage() {
  ...
  const shown = !data
    ...
    : data.ledger.filter((r) => {
        ...
        return r.kind === 'income' || r.kind === 'expense';
      });
      ...
      <div className="space-y-4">
        ...
        {!data && !error ? (
          ...
          <div className="card ruled py-0">
            {shown.map((r, i) => <LedgerRow key={`${r.kind}-${r.ref}-${i}`} row={r} />)}
          </div>
        ) : (
          <Empty
            icon={IconList}
            title="Nothing in this window"
            hint="Every swipe, settlement, loan, repayment, income and expense lands here. Widen the dates to see more."
          />
        )}
      </div>
    </AppShell>
